  ...
  var AirlinesList = [Int : Airlines]()
    ...
    func displayFlight(){
      print("Flight Details")
        //Util.drawLine()
        print("\t ID \t\t Flight ID \t\t\t\t Flight From \t\t Flight To \t\t flightschdate")
        for (_, value) in self.FlightList.sorted(by: { $0.key < $1.key }){
           // Util.drawLine()
            print("\t \(value.FlightID!) ------ \(value.Flightfrom) ------ \(value.Flightto) ------ \(value.Flightschdate!) ------ \(value.FlightairlineID!)  ------ \(value.FlightairplaneID!) ------ \(value.FlightpilotID!) ------ \(value.Flighttypelist!)")
        }
       // Util.drawLine()
    }
