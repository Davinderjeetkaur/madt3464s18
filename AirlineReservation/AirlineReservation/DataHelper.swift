...
class DataHelper{
    ...
    func displayPassenger(){
        print("Passenger Details")
         Util.drawLine()
              print("\t\t Passenger ID \t\t Passenger Name \t\t Address \t\t Passengerpassportnumber ")
        for (_, value) in self.PassengerList.sorted(by: { $0.key < $1.key }){
            Util.drawLine()
                print("\t \(value.PassengerID!) ----------- \(value.PassengerName) --------- \(value.Address) --------- \(value.Passengerpassportnumber!)")
